...
from agents.scout.config import is_enabled, enabled_by_platform
from agents.scout.prompts.scout import SCOUT_PROMPT
from agents.scout.tools.db_tools import (
    get_company_profile,
    save_opportunity,
    list_opportunities,
    get_opportunity,
...
if is_enabled("federal"):
    from agents.scout.tools.sam_api import search_opportunities, get_opportunity_detail
    tools += [search_opportunities, get_opportunity_detail]

# Florida state
if is_enabled("florida_state"):
    from agents.scout.tools.florida_vbs import (
        search_florida_bids,
...
if enabled_by_platform("opengov"):
    from agents.scout.tools.opengov import search_opengov_bids, save_opengov_bids_to_json
...
if enabled_by_platform("bonfire"):
    from agents.scout.tools.bonfire import search_bonfire_bids, save_bonfire_bids_to_json
    tools += [search_bonfire_bids, save_bonfire_bids_to_json]

# Simple HTML scrapers
if enabled_by_platform("simple"):
    from agents.scout.tools.safety_harbor import (
